...
class TemporalGroupCV(BaseCrossValidator):
    ...
    def _temporal_split(self, df: pl.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for region_id in unique_regions:
            ...
            intervals = (
                plot_df.group_by([self.time_start, self.time_end])
        ...
        train_mask = df_with_idx["period_idx"] <= (
            df_with_idx["tot_period_idx"] * self.train_ratio
        )
        test_mask = df_with_idx["period_idx"] > (
            df_with_idx["tot_period_idx"] * self.train_ratio
        )
    ...
    def temp_group_split(
        ...
        for fold, (train_idx, test_idx) in enumerate(
            ...
            if len(temp_train) > 0 and len(temp_test) > 0:
                ...
                print(
                    f"Warning: Empty split in fold {fold} - train: {len(temp_train)}, test: {len(temp_test)}"
                )
...
if __name__ == "__main__":
    ...
    splitter = TemporalGroupCV(
        df=df, train_ratio=0.8, cv=5, group_by=group_by, random_state=42
    )

    for fold, (train_idx, test_idx) in enumerate(
        splitter.temp_group_split(to_numpy(X), to_numpy(y), groups=groups)
    ):
