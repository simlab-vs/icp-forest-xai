...
class HierarchicalTimeGroupCV(BaseCrossValidator):
    ...
    def prepare_cv_data(self, species, ablation="all", tree_group="tree_id", 
                        period_group="period_idx", plot_group="plot_id"):
        """
        ...
        self.logger.info(f"Data prepared: X shape={X.shape if hasattr(X, 'shape') else 'unknown'}, "
                        f"n_samples={len(y)}")
    ...
    def temporal_split(self, X, y, tree_groups, period_group, plot_group):
        ...
        for fold, (train_tree_idx, test_tree_idx) in enumerate(gkf.split(X, y, groups=to_numpy(tree_groups))):
            ...
            for plot_id in unique_plots:
                ...
                plot_splits = list(self.generate_period_splits(periods, math.ceil(len(periods) / 2)))
    ...
    def run_cross_validation(self, species, ablation="all", tree_group="tree_id",
                            period_group="period_idx", plot_group="plot_id"):
        """
        ...
        for fold, (train_idx, test_idx) in enumerate(self.temporal_split(
            to_numpy(X),
            y,
            ...
            self.logger.info(f"Fold {fold} - Train periods: {len(train_idx)}, Test periods: {len(test_idx)}")
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(cv.run_cross_validation(species="spruce")):
