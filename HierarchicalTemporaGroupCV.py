...
class HierarchicalTimeGroupCV(BaseCrossValidator):
    ...
    def temporal_split(self, X, y, tree_groups, period_group, plot_group):
        ...
        for fold, (train_tree_idx, test_tree_idx) in enumerate(
            ...
            print(
                f"Length of train: {len(train_tree_idx)}, Length of test: {len(test_tree_idx)}"
            )
            ...
            for plot_id in unique_plots:
                ...
                plot_splits = list(
                    self.generate_period_splits(periods, round(len(periods) * 0.6))
                )
...
if __name__ == "__main__":
    ...
    for plot_id in df["plot_id"].unique():
        ...
        plot_df = plot_df.with_columns(pl.lit(len(intervals)).alias("tot_period_idx"))
        dfs.append(plot_df)
    ...
    cv = HierarchicalTimeGroupCV(n_splits_tree=5)
